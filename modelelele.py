...
class AQIPredictor:
    ...
    def generate_report(self):
        ...
        X, y, pollutant_features = self.prepare_features()
        if X is not None and y is not None:
            st.write(f"**Available pollutant features**: {pollutant_features}")
        ...
        top_features, feature_importance = self.feature_analysis(X, y, pollutant_features)
        ...
        X_train, X_test, y_train, y_test, y_train_pred, y_test_pred = self.train_model(X, y)
...
def main():
    ...
    page = st.sidebar.radio("Go to", ["Home", "Graphs", "View Model Report"], index=0)
